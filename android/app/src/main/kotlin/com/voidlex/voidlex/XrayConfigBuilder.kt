...
internal object XrayConfigBuilder {
    ...
    internal fun buildWithInbounds(
        ...
        return JSONObject().apply {
            ...
            put("policy", buildPolicy(config.connectionPolicy))
            if (dns != null) {
                put("dns", dns)
            }
    ...
    private fun buildProxyCatchAllRule(): JSONObject {
        return JSONObject().apply {
            put("type", "field")
            // A "field" rule needs at least one matcher or Xray rejects the
            // whole config at startup with "this rule has no effective fields"
            // (router config Build). `network: tcp,udp` matches every flow
            // Xray can route — it only handles tcp/udp — so it behaves as a
            // true catch-all while still counting as an effective field.
            put("network", "tcp,udp")
    ...
    internal fun buildSniffing(routeOnly: Boolean = true): JSONObject {
        return JSONObject().apply {
            put("enabled", true)
    ...
    internal fun resolveBindHost(config: ServerConfig): String {
        return if (config.hotspotBindEnabled || config.runMode == RunMode.PROXY_ONLY) {
            RuntimePorts.HOTSPOT_BIND_HOST
        } else {
            RuntimePorts.XRAY_SOCKS_HOST
        }
    }

    internal fun buildSocksInbound(config: ServerConfig): JSONObject {
        ...
        return JSONObject().apply {
            ...
            if (config.tunnelNetworkSettings.packetAnalysisEnabled) {
                put("sniffing", buildSniffing(config.tunnelNetworkSettings.sniffingRouteOnly))
            }
        }
    }
    ...
    internal fun buildPolicy(policy: ConnectionPolicy): JSONObject {
        val normalized = policy.normalized()
        return JSONObject().apply {
            put("levels", JSONObject().apply {
                put("0", JSONObject().apply {
                    put("handshake", normalized.handshakeSeconds)
                    put("connIdle", normalized.connIdleSeconds)
                    put("uplinkOnly", normalized.uplinkOnlySeconds)
                    put("downlinkOnly", normalized.downlinkOnlySeconds)
                    put("bufferSize", 4)
                })
            })
