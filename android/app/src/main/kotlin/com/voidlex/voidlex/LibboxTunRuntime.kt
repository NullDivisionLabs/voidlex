...
internal class LibboxTunRuntime(
    ...
    override fun openTun(options: TunOptions): Int {
        ...
        val underlyingNetwork = if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            requireUnderlyingNetwork()
        } else {
            null
        }
        if (underlyingNetwork != null) {
            ...
            AppLogger.w(
                TAG,
                "openTun: no underlying network resolved — Android may fall back to system default",
            )
        }
        ...
        while (inet4Address.hasNext()) {
            ...
            addedAddresses.add("${addr.address()}/${addr.prefix()}")
        }
        ...
        AppLogger.i(TAG, "openTun addresses=${addedAddresses.joinToString(",")}")
        ...
        if (options.autoRoute) {
            ...
            val installedDns = mutableListOf<String>()
            dnsServerAddresses
                .filter { it.isNotBlank() }
                .forEach { dnsServerAddress ->
                    runCatching {
                        builder.addDnsServer(dnsServerAddress)
                        installedDns.add(dnsServerAddress)
                    }.onFailure {
                        AppLogger.w(TAG, "addDnsServer failed for $dnsServerAddress", it)
                    }
                }
            AppLogger.i(TAG, "openTun dns=${installedDns.joinToString(",")}")

            val addedRoutes = mutableListOf<String>()
            ...
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                ...
                if (inet4RouteAddress.hasNext()) {
                    while (inet4RouteAddress.hasNext()) {
                        ...
                        addedRoutes.add("${route.address()}/${route.prefix()}")
                    }
                } else {
                    builder.addRoute("0.0.0.0", 0)
                    addedRoutes.add("0.0.0.0/0")
                }
                ...
                while (inet4RouteExcludeAddress.hasNext()) {
                    ...
                    excludedRoutes.add("${route.address()}/${route.prefix()}")
                }
            ...
            AppLogger.i(
                TAG,
                "openTun routes added=${addedRoutes.joinToString(",")} " +
                    "excluded=${excludedRoutes.joinToString(",").ifEmpty { "-" }}",
            )
            ...
            if (includePackages.isNotEmpty()) {
                AppLogger.i(
                    ...
                    "openTun package routing: mode=allow, include=${includePackages.size} " +
                        "packages=${includePackages.joinToString(",")}",
                )
                ...
                AppLogger.i(
                    TAG,
                    "openTun package routing: mode=disallow, " +
                        "excludeRaw=${rawExcludePackages.size}, " +
                        "excludeApplied=${excludePackages.size} " +
                        "packages=${excludePackages.joinToString(",")}",
                )
                ...
                if (rejected.isNotEmpty()) {
                    AppLogger.i(
                        TAG,
                        "openTun rejected (NameNotFound) " +
                            "packages=${rejected.joinToString(",")}",
                    )
                }
            }
        }

        AppLogger.i(TAG, "openTun about to call Builder.establish()")
        val pfd = builder.establish() ?: run {
            AppLogger.e(TAG, "openTun Builder.establish() returned null — no PFD")
            error("Failed to establish TUN")
        }
        ...
        AppLogger.i(
            TAG,
            "openTun TUN established: fd=${pfd.fd} mtu=$effectiveMtu " +
                "autoRoute=${options.autoRoute} underlay=${underlyingNetwork ?: "-"}",
        )
        return pfd.fd
    }
