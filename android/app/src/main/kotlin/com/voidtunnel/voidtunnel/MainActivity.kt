...
class MainActivity: FlutterActivity() {
    ...
    private val CHANNEL = "org.voidtunnel.vpn/service"
    private val STATE_CHANNEL = "org.voidtunnel.vpn/state"
    private val GEODATA_PROGRESS_CHANNEL = "org.voidtunnel.vpn/geodata_progress"
    private val DEEPLINK_CHANNEL = "void.deeplink"
    private val DEEPLINK_EVENT_CHANNEL = "void.deeplink/events"
    private val TV_INFO_CHANNEL = "org.voidtunnel.tv/info"
    ...
    override fun configureFlutterEngine(@NonNull flutterEngine: FlutterEngine) {
        ...
        // If the activity was started by a voidtunnel:// VIEW intent, push
        ...
        intent?.let { handleVoidTunnelIntent(it, isInitial = true) }
        ...
        MethodChannel(flutterEngine.dartExecutor.binaryMessenger, CHANNEL).setMethodCallHandler {
            ...
            when (call.method) {
                ...
                "refreshAndroidWidgets" -> {
                    VoidTunnelWidgetUpdater.requestUpdate(applicationContext)
                    result.success(null)
                }
    ...
    private fun handleSecureSetString(args: Map<*, *>?, result: MethodChannel.Result) {
        ...
        SecurePrefsBridge.set(applicationContext, key, value)
        result.success(null)
    }
    ...
    private fun handleSecureRemove(key: String?, result: MethodChannel.Result) {
        ...
        SecurePrefsBridge.remove(applicationContext, key)
        result.success(null)
    }
    ...
    private fun handleStopVpn(result: MethodChannel.Result) {
        try {
            val stopIntent = Intent(this, VoidVpnService::class.java).apply {
    ...
    override fun onNewIntent(intent: Intent) {
        ...
        setIntent(intent)
        handleVoidTunnelIntent(intent, isInitial = false)
    }

    /**
     * Inspects [intent] for a voidtunnel:// VIEW action and forwards the URL
     * to [DeepLinkBridge] if present. No-op otherwise.
     */
    private fun handleVoidTunnelIntent(intent: Intent, isInitial: Boolean) {
        if (intent.action != Intent.ACTION_VIEW) return
        val data: Uri = intent.data ?: return
        if (!"voidtunnel".equals(data.scheme, ignoreCase = true)) return
