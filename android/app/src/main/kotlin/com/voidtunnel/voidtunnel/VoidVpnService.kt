...
class VoidVpnService : VpnService() {
    ...
    private fun ensureNotificationChannel() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            val channelId = "void_tunnel_channel"
    ...
    /**
     * Polls per-UID byte counters once per second and republishes the
     * foreground notification with an instantaneous up/down rate.
     *
     ...
     */
    private fun wireSpeedTelemetryIfNeeded() {
        if (!showSpeedInNotification) return
        speedPollJob?.cancel()
        lastUidRx = -1L
        lastUidTx = -1L
        lastTrafficAtElapsed = 0L
        ...
        speedPollJob = serviceScope.launch {
            while (isActive) {
                val line = sampleUidTrafficLine()
                if (line != null) {
                    val srv = serverConfig?.server
                    if (srv != null) {
                        val text = "$srv \u2022 $line"
                        val now = SystemClock.elapsedRealtime()
                        // Skip the NotificationManager.notify Binder hop when
                        // the rendered line hasn't actually changed, but
                        // still refresh once every NOTIF_REFRESH_INTERVAL_MS
                        // so the notification doesn't go stale on long-idle
                        // sessions (some OEM launchers age out unchanged
                ...
                delay(1_000)
                if (!showSpeedInNotification) break
            }
        }
    }

    private fun sampleUidTrafficLine(): String? {
        ...
        lastUidTx = tx
        lastTrafficAtElapsed = now
        val downBps = drx / dtSec
        val upBps = dtx / dtSec
        return "\u2191 ${formatBps(upBps)} \u2193 ${formatBps(downBps)}"
    }
