...
class XrayConfigBuilderTest {
    ...
    fun `proxy-selected app routing takes priority over user direct rules`() {
        ...
        // The proxy catch-all must carry a matcher field, otherwise Xray
        // rejects the config with "this rule has no effective fields". We use
        // `network: tcp,udp` as the universal matcher.
        assertTrue(
            routeRules.any {
                it.optString("outboundTag") == "proxy" &&
                    it.optString("network") == "tcp,udp" &&
                    !it.has("domain") &&
                    !it.has("ip") &&
                    !it.has("port") &&
                    !it.has("protocol")
